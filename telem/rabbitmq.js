...
class RabbitMQ {
  ...
  constructor(ip, port, queuesNames, socketClient, redisPersonal) {
    ...
    this.socketClient.socket.on("trackdrone", (data, callback) => {
      const { droneid, baseresponseid } = data;
      console.log("sending command to track ");
      console.log(droneid, baseresponseid);
      this.sendMessage(droneid, {
        action: "trackdrone",
        payload: { responseid: baseresponseid },
    ...
    this.socketClient.socket.on("changethermalpalette", (data, callback) => {
      const { droneid, baseresponseid, palette } = data;
      // console.log(auto_exposureEx,exposure_time)

      console.log("changing thermal palette");

      this.sendMessage(droneid, {
        action: "changethermalpalette",
        payload: {
          responseid: baseresponseid,
          palette,
        },
    ...
    this.socketClient.socket.on("movedronetodistance", (data, callback) => {
      const temp = { ...data };
      const droneid = temp.droneid;
      const responseid = temp.baseresponseid;
      console.log("mode drone to ditance", droneid);
      delete temp.droneid;
      delete temp.baseresponseid;
      temp.responseid = responseid;
      // this.io.to(data.droneid).emit("movedrone", temp);
      this.sendMessage(droneid, {
  ...
  async connect() {
    try {
      this.connection = await amqp.connect(
        `amqp://user:password@${this.ip}:${this.port}`
      );
